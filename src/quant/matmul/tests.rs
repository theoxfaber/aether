        ...
        for col_b in 0..n {
            ...
            for block_idx in 0..blocks_per_row {
                ...
                for _j in (0..Q4K_BLOCK_SIZE).step_by(64) {
                    ...
                    let d_a1 = half::f16::from_le_bytes([a_quant[start_lo], a_quant[start_lo + 1]])
                        .to_f32();
                    let d_a2 = half::f16::from_le_bytes([a_quant[start_hi], a_quant[start_hi + 1]])
                        .to_f32();
            ...
            for block_idx in 0..blocks_per_row {
                ...
                for half_idx in 0..2 {
                    ...
                    for l in (0..32).step_by(16) {
                        ...
                        let start_ao2 = (row_a * blocks_per_row * 8 + b_base + 1) * Q8_BLOCK_BYTES;
                        let start_ao3 = (row_a * blocks_per_row * 8 + b_base + 2) * Q8_BLOCK_BYTES;
                        let start_ao4 = (row_a * blocks_per_row * 8 + b_base + 3) * Q8_BLOCK_BYTES;

                        let d_a1 =
                            half::f16::from_le_bytes([a_quant[start_ao1], a_quant[start_ao1 + 1]])
                                .to_f32();
                        let d_a2 =
                            half::f16::from_le_bytes([a_quant[start_ao2], a_quant[start_ao2 + 1]])
                                .to_f32();
                        let d_a3 =
                            half::f16::from_le_bytes([a_quant[start_ao3], a_quant[start_ao3 + 1]])
                                .to_f32();
                        let d_a4 =
                            half::f16::from_le_bytes([a_quant[start_ao4], a_quant[start_ao4 + 1]])
                                .to_f32();
    ...
    for m in m_cases {
        ...
        if m > 1 {
            ...
            for i in 0..c_scalar.len() {
                ...
                assert!(
                    diff < 1e-5,
                    "Q2_K batched mismatch at index {} for m={}: scalar={}, batched={}, diff={}",
                    i,
                    m,
                    c_scalar[i],
                    c_batched[i],
...
fn test_asymmetric_gguf_order_f16() {
    ...
    let b_f32_gguf =
        crate::loader::dequant::dequantize(&b_bytes, crate::loader::gguf::GGUFDtype::F16, &[k, n]);
    ...
    for m in m_cases {
        ...
        if m > 1 {
            ...
            for i in 0..c_scalar.len() {
                ...
                assert!(
                    diff < 1e-5,
                    "Q3_K batched mismatch at index {} for m={}: scalar={}, batched={}, diff={}",
                    i,
                    m,
                    c_scalar[i],
                    c_batched[i],
